...
| **AM Performance**     | Account Manager rankings (revenue, margin, YoY growth), growth leader leaderboard, state & product champions, per-AM stacked product revenue chart                               |
...
- **AM hover cards** — hover any Account Manager name to see a rich pop-up with:
  - Name, job title, and state badge
  - One-click **email** and **call** (tel:) buttons
  - Revenue, YoY growth, and margin KPIs
  - Per-product revenue breakdown with bar chart
  - Viewport-aware positioning (never clipped at edges)
- Sticky table column headers when scrolling long AM lists
...
- **Account Managers:** 32 real Leader Systems AM names with correct states, job titles, emails, and phone numbers sourced from the internal AM contact list
- **Partners:** 25 sample partner companies
- **Revenue Trend:** 18 months of generated data (Sep 2024 – Feb 2026)
...
│   │   ├── filters/         # GlobalFilters (state, product, period, account manager)
│   │   ├── layout/          # AppShell (sidebar + header + routing shell)
│   │   └── ui/              # KpiCard, Panel, SimpleTable, ProductIcon, AMHoverCard
│   ├── data/
│   │   ├── mockData.ts      # All dummy data (replace with API calls)
│   │   └── account_managers.json  # Real Leader Systems AM contact list
│   ├── pages/
